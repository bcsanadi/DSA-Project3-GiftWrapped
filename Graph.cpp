...
// add product as a node if it does not already exist
void Graph::addNode(const Product& product) {
    if (graph.find(product.asin) == graph.end())
        graph[product.asin] = Node{product};
}

// create directed edge between two product nodes with a given weight
...
// traverse graph starting from list of ASINs and return related products
    ...
    for (const string& asin : userInput) {
        if (graph.find(asin) != graph.end()) {
            for (const auto& neighbor : graph[asin].relatedProducts) {
                ...
                // add to result if product not already visited
...
// build graph by adding nodes and connecting related products based on user inputs
void Graph::buildGraph(const vector<Product>& products, Graph& graph, const string& price, const string& age, const string& relationship) {
    ...
    // connect products based on shared attributes
    for (int i = 0; i < products.size(); i++) {
        for (int j = 0; j < products.size(); j++) {
            int weight = 0;

            // increase weight for shared interest
            if (products[i].interest == products[j].interest)
                weight++;

            // increase weight if both products share the same age group and match user's selected age
            if (products[i].ageGroup == products[j].ageGroup && products[i].ageGroup == age)
                weight++;

            // increase weight if both products share the same relationship and match user's selected relationship
            if (products[i].relationship == products[j].relationship && products[i].relationship == relationship)
                weight++;

            // add bidirectional edges if weight is positive
            if (weight > 0) {
